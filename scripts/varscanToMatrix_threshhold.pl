...
GetOptions ("verbose"           => \$verbose,
            ...
            "minThreshhold=s"   => \$minThreshhold,
            "maxThreshhold=i"   => \$maxThreshhold
      ) or pod2usage(0) && exit;
    ...
    for (my $i = 1; $i < scalar(@sampleList); $i++) {
        for (my $j = 0; $j < $i; $j++) {
            if (exists($resultsHash{$window}{$sampleList[$i]}{sum}) &&
                ...
                # values are set to 2 for duplications, 0 for deletions and 1 otherwise
                $val_i = theshholdvalue($val_i);
                $val_j = theshholdvalue($val_j);
...
sub theshholdvalue {
    ...
    if ($val >= 2**$maxThreshhold) {
        $val = 2;
    } elsif ($val <= 2**$minThreshhold) {
        $val = 0;
    } else {
        $val = 1;
    }
